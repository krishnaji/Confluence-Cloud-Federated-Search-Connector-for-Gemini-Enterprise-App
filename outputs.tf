...
output "federated_search_next_steps" {
  ...
  value       = "After Terraform provisions the federated connector, open the Gemini Enterprise Console under Data Stores, select '${google_discovery_engine_data_connector.confluence_federated_connector.collection_display_name}', and perform the initial OAuth login to complete end-user authorization."
}
